...
type Invoice struct {
	...
	//ReleaseAt Time `json:"release_at,omitempty"`
}
...
type InvoicesCreateOptions struct {
	...
	Description    string          `json:"description"`               // Invoice description that will be displayed to the user
	Currency       string          `json:"currency"`                  // ISO invoice currency that you want to receive from the user, for example: "EUR”
	SenderCurrency *string         `json:"sender_currency,omitempty"` // Currency of user invoice payment (3rd type invoice will be externalized at the time of sending this parameter with timer= true), example: "BTC"
	Amount         decimal.Decimal `json:"amount"`                    // Invoice amount that you want to receive from the user
	ForeignID      string          `json:"foreign_id,omitempty"`      // Unique foreign ID in your system, example: "164"
	URLSuccess     string          `json:"url_success,omitempty"`     // URL on which we redirect the user in case of a successful invoice payment, example: "https://merchant.name.com/url_success"
	URLFailed      string          `json:"url_failed,omitempty"`      // URL on which we redirect the user in case of an unsuccessful invoice payment, example: "https://merchant.name.com/url_failed"
	...
	//Type           string          `json:"type,omitempty"`            // Type of invoice. Possible values: "fixed" - fixed amount invoice, "dynamic" - dynamic amount invoice, "third_party" - third party invoice
}
...
func (s *InvoicesService) Get(ctx context.Context, id int64) (*Invoice, error) {
	req, err := s.client.newRequest(http.MethodGet, "invoices/get", struct {
		ID int64 `json:"id"`
	}{ID: id})
